...
export default function SafetyCommitmentPage() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(135deg,#042c53 0%,#0c447c 50%,#042c53 100%)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'24px 16px', fontFamily:"'Segoe UI',sans-serif", position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ maxWidth:860, width:'100%', opacity:visible?1:0, transform:visible?'translateY(0)':'translateY(20px)', transition:'all 0.6s ease' }}>

        <div style={{ textAlign:'center', marginBottom:24 }}>
          <img src="/logo.png" alt="Egypro" style={{ height:56, marginBottom:12 }} onError={e=>{e.target.style.display='none';}} />
        ...
        <div style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderLeft:'3px solid #1d9e75', borderRadius:8, padding:'14px 20px', marginBottom:20, textAlign:'center' }}>
          <p style={{ fontSize:13, color:'rgba(255,255,255,0.85)', lineHeight:1.7, margin:0, fontStyle:'italic' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:20 }}>
          {RULES.map((rule, i) => (
            <div key={rule.num} style={{
              background: rule.type==='NEVER' ? 'rgba(226,75,74,0.08)' : 'rgba(255,255,255,0.05)',
              border: rule.type==='NEVER' ? '1px solid rgba(226,75,74,0.25)' : '1px solid rgba(255,255,255,0.08)',
              ...
              <img src={rule.icon} alt={'Rule '+rule.num} style={{ width:52, height:52, objectFit:'contain' }} />
              <div style={{ fontSize:9, fontWeight:800, letterSpacing:1.5, color:rule.type==='NEVER'?'#e24b4a':'#1d9e75', textTransform:'uppercase' }}>{rule.type}</div>
              <div style={{ fontSize:11, color:'rgba(255,255,255,0.8)', lineHeight:1.5 }}>{rule.text}</div>
            </div>
          ))}
        </div>
