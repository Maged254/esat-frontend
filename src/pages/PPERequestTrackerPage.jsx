    ...
    <>
      ...
      <div className="content">
        ...
        <div className="stat-grid" style={{marginBottom:16,gridTemplateColumns:'repeat(6,1fr)'}}>
          ...
          <div className="stat-card wf-stat-card" style={{cursor:'pointer',borderTopColor:'var(--wf-ehs)',background:filters.status==='pending'?'var(--wf-ehs-light)':'',outline:filters.status==='pending'?'2px solid var(--wf-ehs)':'',boxShadow:filters.status==='pending'?'var(--wf-shadow-hover)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='pending'?'':'pending'})); setGroupMode('none');}}><div className="stat-label">Pending EHS</div><div className="stat-value" style={{color:'var(--wf-ehs)'}}>{requests.filter(r=>r.status==='pending').length}</div></div>
          <div className="stat-card wf-stat-card" style={{cursor:'pointer',borderTopColor:'var(--wf-pm)',background:filters.status==='pda_pending'?'var(--wf-pm-light)':'',outline:filters.status==='pda_pending'?'2px solid var(--wf-pm)':'',boxShadow:filters.status==='pda_pending'?'var(--wf-shadow-hover)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='pda_pending'?'':'pda_pending'})); setGroupMode('none');}}><div className="stat-label">Pending PM</div><div className="stat-value" style={{color:'var(--wf-pm)'}}>{requests.filter(r=>r.status==='ehs_purchase_requested' && r.needs_pda).length}</div></div>
          <div className="stat-card wf-stat-card" style={{cursor:'pointer',borderTopColor:'var(--wf-scm)',background:filters.status==='ehs_purchase_requested'?'var(--wf-scm-light)':'',outline:filters.status==='ehs_purchase_requested'?'2px solid var(--wf-scm)':'',boxShadow:filters.status==='ehs_purchase_requested'?'var(--wf-shadow-hover)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='ehs_purchase_requested'?'':'ehs_purchase_requested'})); setGroupMode('none');}}><div className="stat-label">Pending SCM</div><div className="stat-value" style={{color:'var(--wf-scm)'}}>{requests.filter(r=>r.status==='ehs_purchase_requested' && !(r.needs_pda && !r.pda_approved_date)).length}</div></div>
          <div className="stat-card wf-stat-card" style={{cursor:'pointer',borderTopColor:'var(--wf-scm)',background:filters.status==='scm_ordered'?'var(--wf-scm-light)':'',outline:filters.status==='scm_ordered'?'2px solid var(--wf-scm)':'',boxShadow:filters.status==='scm_ordered'?'var(--wf-shadow-hover)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='scm_ordered'?'':'scm_ordered'})); setGroupMode('none');}}><div className="stat-label">Pending Suppliers</div><div className="stat-value" style={{color:'var(--wf-scm)'}}>{requests.filter(r=>r.status==='scm_ordered').length}</div></div>
          <div className="stat-card wf-stat-card" style={{cursor:'pointer',borderTopColor:'var(--wf-projects)',background:filters.status==='warehouse_available'?'var(--wf-projects-light)':'',outline:filters.status==='warehouse_available'?'2px solid var(--wf-projects)':'',boxShadow:filters.status==='warehouse_available'?'var(--wf-shadow-hover)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='warehouse_available'?'':'warehouse_available'})); setGroupMode('none');}}><div className="stat-label">Pending Projects</div><div className="stat-value" style={{color:'var(--wf-projects)'}}>{requests.filter(r=>r.status==='warehouse_available').length}</div></div>
        </div>
        <div className="card">
            ...
            <table style={{tableLayout:'fixed'}}>
              ...
              <thead>
                <tr style={{position:'sticky',top:0,zIndex:4}}>
                  ...
                  <th colSpan={2} style={{textAlign:'center',background:'var(--wf-ehs-gradient)',color:'#ffffff',fontWeight:700,fontSize:11,letterSpacing:1,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb',borderBottom:'none'}}>EHS</th>
                  <th colSpan={1} style={{textAlign:'center',background:'var(--wf-pm-gradient)',color:'#ffffff',fontWeight:700,fontSize:11,letterSpacing:1,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb',borderBottom:'none'}}>PM</th>
                  <th colSpan={2} style={{textAlign:'center',background:'var(--wf-scm-gradient)',color:'#ffffff',fontWeight:700,fontSize:11,letterSpacing:1,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb',borderBottom:'none'}}>SCM</th>
                  <th colSpan={1} style={{textAlign:'center',background:'var(--wf-projects-gradient)',color:'#ffffff',fontWeight:700,fontSize:11,letterSpacing:1,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb',borderBottom:'none'}}>Projects</th>
