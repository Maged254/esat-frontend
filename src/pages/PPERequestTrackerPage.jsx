    ...
    <>
      ...
      <div className="content">
        <div className="stat-grid" style={{marginBottom:16,gridTemplateColumns:'repeat(6,1fr)'}}>
          ...
          <div className="stat-card" style={{cursor:'pointer',outline:filters.status==='ehs_purchase_requested'?'2px solid var(--eg-green)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='ehs_purchase_requested'?'':'ehs_purchase_requested'})); if(filters.status==='ehs_purchase_requested') setGroupPO(false);}}><div className="stat-label">Pending SCM</div><div className="stat-value navy">{requests.filter(r=>r.status==='ehs_purchase_requested').length}</div></div>
              ...
              <tbody>
                {groupPO && filters.status === 'ehs_purchase_requested' ? (() => {
                  ...
                  <tr key={r.id} style={{background: bulkTarget && isEligible(r) ? 'rgba(29,158,117,0.05)' : ''}}>
                    ...
                    <td style={{fontSize:12,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb'}}>{r.pda_approved_date?<div><div>{new Date(r.pda_approved_date).toLocaleDateString('en-GB')}</div>{r.pda_approved_by_name&&<div style={{fontSize:10,color:'#6b7280',marginTop:2}}>{r.pda_approved_by_name}</div>}</div>:(r.needs_pda?<span style={{fontSize:10,color:'#9333ea'}}>Pending</span>:'N/A')}</td>
