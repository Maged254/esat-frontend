            ...
            <table style={{borderCollapse:'separate',borderSpacing:0}}>
              <thead>
                <tr>
                  ...
                  <th colSpan={2} style={{textAlign:'center',background:'#e6f1fb',color:'#0c447c',fontWeight:700,fontSize:11,letterSpacing:1,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb',borderTop:'1px solid #e5e7eb',position:'sticky',top:0,zIndex:3}}>EHS</th>
                  <th colSpan={1} style={{textAlign:'center',background:'#f3e8ff',color:'#6d28d9',fontWeight:700,fontSize:11,letterSpacing:1,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb',borderTop:'1px solid #e5e7eb',position:'sticky',top:0,zIndex:3}}>PM</th>
                  <th colSpan={2} style={{textAlign:'center',background:'#e8f5e9',color:'#1d9e75',fontWeight:700,fontSize:11,letterSpacing:1,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb',borderTop:'1px solid #e5e7eb',position:'sticky',top:0,zIndex:3}}>SCM</th>
                  <th colSpan={1} style={{textAlign:'center',background:'#fff3e0',color:'#e65100',fontWeight:700,fontSize:11,letterSpacing:1,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb',borderTop:'1px solid #e5e7eb',position:'sticky',top:0,zIndex:3}}>Projects</th>
                ...
                {groupMode==='po' ? (() => {
                  ...
                  clients.forEach(client => {
                    ...
                    itemOrder.forEach(key => {
                      ...
                      g.rows.forEach(r => rows.push(<tr key={r.id} style={{background:bulkTarget&&isEligible(r)?'rgba(29,158,117,0.05)':''}}><td style={{position:"sticky",left:0,zIndex:1,background:"#fff",minWidth:160,maxWidth:160,borderBottom:"1px solid #e5e7eb",borderRight:"1px solid #e5e7eb"}}><div className="emp-name">{r.employee_name}</div><div className="emp-id">{r.employee_national_id||r.employee_number}</div>{r.job_title&&<div style={{fontSize:10,color:'#6b7280',marginTop:1}}>{r.job_title}</div>}</td><td style={{position:"sticky",left:160,zIndex:1,background:"#fff",minWidth:220,maxWidth:220,boxShadow:"2px 0 4px rgba(0,0,0,0.06)",borderBottom:"1px solid #e5e7eb",borderRight:"1px solid #e5e7eb"}}>{r.ppe_name}</td><td>{r.size_value||'—'}</td><td style={{fontSize:12,color:(r.quantity||1)>1?'#e53e3e':'inherit',fontWeight:(r.quantity||1)>1?700:400}}>{r.quantity||1}</td><td style={{fontSize:12}}>{r.location_name||'—'}</td><td style={{fontSize:12}}><div>{r.project||'—'}</div>{r.client && <div style={{fontSize:10,color:'#6b7280',marginTop:2}}>{r.client}</div>}</td><td style={{fontSize:12,borderLeft:'1px solid #e5e7eb'}}>{r.date_flagged?<div><div>{new Date(r.date_flagged).toLocaleDateString('en-GB')}</div>{r.flagged_by_name&&<div style={{fontSize:10,color:'#6b7280',marginTop:2}}>{r.flagged_by_name}</div>}</div>:'—'}</td><td style={{fontSize:12,borderRight:'1px solid #e5e7eb'}}>{r.date_purchase_requested?<div><div>{new Date(r.date_purchase_requested).toLocaleDateString('en-GB')}</div>{r.purchase_requested_by_name&&<div style={{fontSize:10,color:'#6b7280',marginTop:2}}>{r.purchase_requested_by_name}</div>}</div>:'—'}</td><td style={{fontSize:12,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb'}}>{r.pda_approved_date?<div><div>{new Date(r.pda_approved_date).toLocaleDateString('en-GB')}</div>{r.pda_approved_by_name&&<div style={{fontSize:10,color:'#6b7280',marginTop:2}}>{r.pda_approved_by_name}</div>}</div>:(r.needs_pda?'—':'N/A')}</td><td style={{fontSize:12,borderLeft:'1px solid #e5e7eb'}}>{r.date_ordered?<div><div>{new Date(r.date_ordered).toLocaleDateString('en-GB')}</div>{r.ordered_by_name&&<div style={{fontSize:10,color:'#6b7280',marginTop:2}}>{r.ordered_by_name}</div>}{r.po_number&&<div style={{fontSize:10,fontWeight:700,color:'#0f2a4a',marginTop:2}}>{r.po_number}</div>}</div>:'—'}</td><td style={{fontSize:12,borderRight:'1px solid #e5e7eb'}}>{r.date_available?<div><div>{new Date(r.date_available).toLocaleDateString('en-GB')}</div>{r.available_by_name&&<div style={{fontSize:10,color:'#6b7280',marginTop:2}}>{r.available_by_name}</div>}</div>:'—'}</td><td style={{fontSize:12,borderLeft:'1px solid #e5e7eb',borderRight:'1px solid #e5e7eb'}}>{r.date_distributed?<div><div>{new Date(r.date_distributed).toLocaleDateString('en-GB')}</div>{r.distributed_by_name&&<div style={{fontSize:10,color:'#6b7280',marginTop:2}}>{r.distributed_by_name}</div>}{r.distribution_method==='courier'&&<span onClick={()=>setTrackingModal(r.courier_tracking_number||'No tracking number entered')} style={{display:'inline-block',marginTop:4,fontSize:10,fontWeight:700,background:'#fff3e0',color:'#e65100',border:'1px solid #ffcc80',borderRadius:4,padding:'1px 6px',cursor:'pointer'}}>Courier ›</span>}</div>:'—'}</td><td><span className={'tag '+((r.status==='ehs_purchase_requested'&&r.needs_pda)?'tag-purple':(STATUS_COLORS[r.status]||'tag-gray'))}>{(r.status==='ehs_purchase_requested'&&r.needs_pda)?'Pending PM':(STATUS_LABELS[r.status]||r.status)}</span></td>{bulkTarget&&<td style={{textAlign:'center'}}>{isEligible(r)&&<input type="checkbox" checked={selected.includes(r.id)} onChange={()=>toggleSelect(r.id)} style={{width:16,height:16,cursor:'pointer',accentColor:'var(--eg-green)'}} />}</td>}</tr>));
                    });
                    });
                  });
                  return rows;
                  ...
                  <tr key={r.id} style={{background: bulkTarget && isEligible(r) ? 'rgba(29,158,117,0.05)' : ''}}>
                    ...
                    <td style={{position:"sticky",left:160,zIndex:1,background:"#fff",minWidth:220,maxWidth:220,boxShadow:"2px 0 4px rgba(0,0,0,0.06)",borderBottom:"1px solid #e5e7eb",borderRight:"1px solid #e5e7eb"}}>{r.ppe_name}</td>
                    <td>{r.size_value || '—'}</td>
