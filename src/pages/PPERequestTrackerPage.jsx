    ...
    <>
      ...
      <div className="content">
        ...
        <div className="stat-grid" style={{marginBottom:16,gridTemplateColumns:'repeat(6,1fr)'}}>
          <div className="stat-card" style={{cursor:'pointer',borderLeft:'6px solid var(--eg-navy)',outline:!filters.status?'2px solid var(--eg-green)':''}} onClick={()=>{setFilters(p=>({...p,status:''})); setGroupMode('none');}}><div className="stat-label">Total Requested</div><div className="stat-value navy">{filtered.length}</div></div>
          <div className="stat-card" style={{cursor:'pointer',borderLeft:'6px solid #bfdbfe',outline:filters.status==='pending'?'2px solid var(--eg-green)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='pending'?'':'pending'})); setGroupMode('none');}}><div className="stat-label">Pending EHS</div><div className="stat-value" style={{color:'#1e40af'}}>{requests.filter(r=>r.status==='pending').length}</div></div>
          <div className="stat-card" style={{cursor:'pointer',borderLeft:'6px solid #e9d5ff',outline:filters.status==='pda_pending'?'2px solid var(--eg-green)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='pda_pending'?'':'pda_pending'})); setGroupMode('none');}}><div className="stat-label">Pending PM</div><div className="stat-value" style={{color:'#6d28d9'}}>{requests.filter(r=>r.status==='ehs_purchase_requested' && r.needs_pda).length}</div></div>
          <div className="stat-card" style={{cursor:'pointer',borderLeft:'6px solid #a7f3d0',outline:filters.status==='ehs_purchase_requested'?'2px solid var(--eg-green)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='ehs_purchase_requested'?'':'ehs_purchase_requested'})); setGroupMode('none');}}><div className="stat-label">Pending SCM</div><div className="stat-value" style={{color:'#065f46'}}>{requests.filter(r=>r.status==='ehs_purchase_requested' && !(r.needs_pda && !r.pda_approved_date)).length}</div></div>
          <div className="stat-card" style={{cursor:'pointer',borderLeft:'6px solid #a7f3d0',outline:filters.status==='scm_ordered'?'2px solid var(--eg-green)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='scm_ordered'?'':'scm_ordered'})); setGroupMode('none');}}><div className="stat-label">Pending Suppliers</div><div className="stat-value" style={{color:'#065f46'}}>{requests.filter(r=>r.status==='scm_ordered').length}</div></div>
          <div className="stat-card" style={{cursor:'pointer',borderLeft:'6px solid #fed7aa',outline:filters.status==='warehouse_available'?'2px solid var(--eg-green)':''}} onClick={()=>{setFilters(p=>({...p,status:p.status==='warehouse_available'?'':'warehouse_available'})); setGroupMode('none');}}><div className="stat-label">Pending Projects</div><div className="stat-value" style={{color:'#9a3412'}}>{requests.filter(r=>r.status==='warehouse_available').length}</div></div>
        </div>
        <div className="card">
